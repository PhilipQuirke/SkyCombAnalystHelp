...
This document does NOT cover how to fly a drone. It assumes you know how to fly your drone.
...
A good option is to fly in the hour before dawn, when there is some light in the sky (for the optical camera), 
...
alterative highly-accurate ground elevation data can automatically then SkyComb Analyst can improve 
...
- <b>Auto</b> : Corrects assuming the drone was on the ground at both the start and end, and gives the best correction. This is the recommended approach.
...
If the CameraDownAngle is not recorded in the flight log (common), SkyComb Analyst can't automatically detect or calculate the angle. 
You can manually specify the CameraDownAngle value in the UI.

If the camera was pointing straight down, then set CameraDownAngle to 90 (not -90) in the SkyComb Analyst UI.
If the camera was pointing straight forward (horizontally), then set CameraDownAngle to 0.
If the camera was pointing half way between straight down and straight forward, then set CameraDownAngle to 45.  
...
Then run the SkyComb Analyst tool and select your new thermal or optical video file (doesn't matter which one). 
...
The drone flight path, speed and altitude will be displayed. The ground and tree-top elevations will also be shown.
...
## Syncing the Drone Video and Flight log
While you might expect the optical video, thermal video and flight data to start at exactly the same instant, 
for some drones this is not the case. In some cases, the video start times differ by up to 1.6 seconds.

SkyComb Analyst can't automatically detect or calculate these different start times (yet!). 
You can calculate the differences manually using SkyComb Analyst and then record the differences in the UI.
...
Set the "Process" to "None". Click "Run" and watch the videos and flight path. 
After a few seconds, you will see the optical video, thermal video, and flight path 
start to turn the corner at slightly different times (because they are not perfectly synchronized). 

Click "Run" again, to re-run the same time period. This time focus on just the thermal and optical videos. 
...
Click "Run" again, to re-run the same time period. This time focus on just the thermal video and the flight path graph. 
The flight path shows the area the drone is videoing as a red rectangle.
If the red rectangle starts turning/rotating roughly half a second before the thermal video,
set the "Flight delay (Secs)" to "0.5". Run the process again and further tune the 
setting until the video and red rectangle are synchronized and turn the corner at the same time.
